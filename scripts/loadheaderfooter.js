$(document).ready(function () {
    ...
    // Kích thước Sidebar (Khớp với CSS trong sidebar.html)
    ...
    // Hàm kiểm tra phim đã thích chưa
    ...
    window.toggleFavorite = (e, id) => {
        e.preventDefault(); 
        e.stopPropagation(); // Ngăn việc click nhầm vào link xem phim
    ...
    $("#sidebar-container").load("components/sidebar.html", function(response, status, xhr) {
        ...
        // Logic ẩn/hiện Sidebar theo màn hình 1280px (Laptop lớn)
    ...
    $(document).on("click", "#toggle-sidebar-btn, #sidebar-toggle-btn", function() {
        ...
        // Dưới 1280px dùng Overlay
        if ($(window).width() < 1280) {
            sidebar.toggleClass("-translate-x-full");
            overlay.toggleClass("hidden");
        } 
        // Trên 1280px dùng Push & Fit
        else {
                ...
                <div class="hero-slide absolute inset-0 w-full h-full transition-opacity duration-1000 ease-in-out ${activeClass}" data-index="${index}">
                    ...
                    <div class="hero-content absolute bottom-12 left-6 md:top-auto md:bottom-16 md:left-12 max-w-xl z-20 pr-4">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <button onclick="window.toggleFavorite(event, ${m.id})" class="w-11 h-11 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center hover:bg-white/20 transition group/fav"><i class="${heartClass} text-lg transition-transform group-hover/fav:scale-110"></i></button>
                        </div>
                    </div>
                </div>`;
    ...
    // --- RENDER CAROUSEL (CÓ NÚT TIM + THÔNG TIN CHI TIẾT) ---
                    ...
                    <a href="movie_detail.html?id=${m.id}" class="block cursor-pointer">
                        <div class="aspect-[2/3] w-full rounded-2xl overflow-hidden bg-gray-800 mb-3 relative border border-white/5 shadow-lg">
                            ...
                            <button onclick="window.toggleFavorite(event, ${m.id})" class="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center hover:bg-white/20 transition z-20 group/heart">
                                <i class="${heartClass} text-sm group-hover/heart:scale-110 transition-transform"></i>
                            </button>
    ...
    // --- RENDER TOP RATED (ĐÃ CẬP NHẬT: KHÔNG CÓ NÚT TIM) ---
    // --- RENDER TOP RATED (ĐÃ NÂNG CẤP GIAO DIỆN SỐ HẠNG) ---
    const renderTopRated = (movies) => {
         let html = "";
         movies.sort((a,b) => b.vote_average - a.vote_average).slice(0, 10).forEach((m, i) => {
             
             // --- LOGIC STYLE SỐ HẠNG MỚI ---
             let rankStyle = "";
             let containerStyle = "w-10"; // Độ rộng vùng chứa số

             if (i === 0) {
                 // Top 1: To nhất, Gradient Tím-Hồng, Bóng đổ tím phát sáng
                 rankStyle = "text-5xl font-black italic text-transparent bg-clip-text bg-gradient-to-t from-purple-600 to-pink-400 drop-shadow-[0_4px_10px_rgba(168,85,247,0.6)]";
                 containerStyle = "w-14"; // Cần rộng hơn chút
             } else if (i === 1) {
                 // Top 2: Nhỏ hơn Top 1, Gradient Xanh Cyan (Tương phản đẹp), Bóng nhẹ
                 rankStyle = "text-4xl font-black italic text-transparent bg-clip-text bg-gradient-to-t from-blue-600 to-cyan-400 drop-shadow-md";
                 containerStyle = "w-12";
             } else if (i === 2) {
                 // Top 3: Gradient Vàng/Cam (Màu đồng), Bóng nhẹ
                 rankStyle = "text-4xl font-black italic text-transparent bg-clip-text bg-gradient-to-t from-yellow-700 to-yellow-300 drop-shadow-md";
                 containerStyle = "w-12";
             } else {
                 // Top 4 trở đi: Màu xám, in đậm, chìm hơn
                 rankStyle = "text-2xl font-bold text-gray-600 font-[Outfit]";
             }
             ...
             html += `
                <div class="relative group block mb-3 pl-2">
                    <a href="movie_detail.html?id=${m.id}" class="top-rated-item flex items-center gap-3 p-3 rounded-2xl cursor-pointer bg-[#1e1e1e]/50 border border-white/5 relative overflow-hidden hover:bg-white/5 transition-colors pr-4">
                        
                        <div class="${containerStyle} flex-shrink-0 flex justify-center items-center z-10">
                            <span class="${rankStyle} leading-none" style="font-family: 'Outfit', sans-serif;">${i+1}</span>
                        </div>
